...
const PRINTIFY_SHOP_ID = process.env.PRINTIFY_SHOP_ID || '28919161';
  ...
  try {
    ...
    const session = await stripe.checkout.sessions.create({
      ...
      shipping_address_collection: {
        allowed_countries: ['US', 'CA', 'GB', 'IE', 'AU', 'NZ', 'DE', 'FR', 'NL', 'SE', 'NO', 'DK'],
      },
      line_items: [
        {
          price_data: {
            ...
            product_data: {
              ...
              description: product.description ? product.description.substring(0, 250) : 'Official Aoife Kane Merchandise',
...
app.post('/api/merch/order', async (req, res) => {
  try {
    ...
    const orderPayload = {
      ...
      label: `Aoife Kane Clan Order - ${session.id.substring(0, 10)}`,
      line_items: [
        {
...
app.listen(PORT, () => {
  console.log(`Aoife Kane official server running on http://localhost:${PORT}`);
});
